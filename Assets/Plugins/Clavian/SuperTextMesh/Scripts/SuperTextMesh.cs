...
//and it'll be easier for devs to use, too
	...
	private static List<char> linestartUnfriendlyChars = new List<char>(){'!','%',')',',','.',':',';','?',']','}','¢','°','·','\'','\"','†','‡','›','℃','∶','、','。','〃','〆','〕','〗','〞','﹚','﹜','！','＂','％','＇','）','，','．','：','；','？','！','］','｝','～', //simplified chinese
...
*/
	...
	IEnumerator WaitFrameThenRebuild(bool auto){
		yield return null;
		Rebuild(currentReadTime, auto);
	}
	IEnumerator WaitFrameThenRebuild(bool auto, bool execute){
		yield return null;
		Rebuild(currentReadTime, auto, execute);
	}
...
*/
					...
					{ //split it here! but not if it's the first character
						...
						if(!breakText) //dont format if told to not format!
						{
							int cleanBreak = -1;
							...
							{
								i = Mathf.Min(cleanBreak, dirtyBreak);
							}
							else if(cleanBreak > -1)
							{
								i = cleanBreak; //use clean
							}
						}
						...
						if(insertHyphens){
							...
							infoCount++;
							//use info[i] here instead of Rebuild_info since the info value is changing.
							info[i].UpdateCachedValuesIfChanged(fontTextureJustRebuilt);
	//						SetTextGenSettings(Rebuild_info, i);
...
Vector3 JitterValue_MyJit = Vector3.zero;
	...
	*/
	...
	{
		if(gameObject.activeInHierarchy && uiMode) Rebuild(currentReadTime, currentReadTime > 0f ? true : autoRead);
	}
