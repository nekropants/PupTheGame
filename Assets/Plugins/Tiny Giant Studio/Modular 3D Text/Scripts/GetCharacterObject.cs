...
namespace TinyGiantStudio.Text
{
    public class GetCharacterObject
    {
        ...
        /// <returns></returns>
        public static MeshLayout GetMeshLayout(char c, Modular3DText text, MeshPostProcess meshPostProcess)
        {
            if (!IsSpecialSymbol(c))
                return ProcessNormalCharacter(c, text, meshPostProcess); //stuff with mesh
            else
                return ProcessSpecialCharacter(c, text); //stuff like spaces and tabs, newlines
        }
        ...
        static void SetParent(Modular3DText text, GameObject obj)
        {
            if (text)
            {
                if (obj)
        ...
        static MeshLayout ProcessSpecialCharacter(char c, Modular3DText text)
        {
