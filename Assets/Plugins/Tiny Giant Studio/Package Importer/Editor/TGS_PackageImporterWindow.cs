...
namespace TinyGiantStudio.PackageImporter
{
    internal class TGS_PackageImporterWindow : EditorWindow
    {
        ...
        void DrawPacakge(TGS_Package asset, bool correctPipeline = false)
        {
            ...
            if (asset.lastInstalledVersion == 0) { }
            //EditorGUILayout.LabelField("No previous installation was recorded", ignoreUpdateStyle);
            else
                EditorGUILayout.LabelField("Last installed version: " + asset.lastInstalledVersion, ignoreUpdateStyle);
        ...
        {
            if (correctPipeline && asset.lastInstalledVersion != asset.currentVersion)
            {
