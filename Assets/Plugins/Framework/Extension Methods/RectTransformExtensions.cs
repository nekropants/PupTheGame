...
public static class RectTransformExtensions
{
    ...
    public static void SetScreenSpaceDimensions(this RectTransform rectTransform, Vector2 dimensions)
    {
        ...
        rectTransform.sizeDelta = new Vector2(dimensions.x / rectTransform.lossyScale.x, dimensions.y / rectTransform.lossyScale.y);
    }
    ...
    public static void SetScreenSpaceHeight(this RectTransform rectTransform, float height)
    {
        Assert.IsTrue(rectTransform.anchorMin.y == rectTransform.anchorMax.y, "RectTransform is being streched on the vertical axis. Setting a vertical height makes no sense with this anchor setup!");

        rectTransform.sizeDelta = new Vector2(rectTransform.sizeDelta.x, height / rectTransform.lossyScale.y);
    }
    ...
    public static void SetScreenSpaceWidth(this RectTransform rectTransform, float width)
    {
        Assert.IsTrue(rectTransform.anchorMin.x == rectTransform.anchorMax.x, "RectTransform is being streched on the horizontal axis. Setting a horizontal width makes no sense with this anchor setup!");

        rectTransform.sizeDelta = new Vector2(width / rectTransform.lossyScale.x, rectTransform.sizeDelta.y);
    }
    ...
    /// </summary>
    /// <param name="fromCorner">If true, return the local position of the bottom left corner of the rectangle. Otherwise return the local position of the rectangle's pivot</param>
    /// <returns>The local position, in local-space pixels</returns>
    public static Vector2 GetLocalPosition(this RectTransform rectTransform, bool fromCorner = false)
    {
        if (fromCorner)
        {
    ...
    /// </summary>
    /// <param name="fromCorner">If true, return the position of the bottom left corner of the rectangle. Otherwise return the position of the rectangle's pivot</param>
    /// <returns>The absoloute position of the rectangle, in screen-space pixels</returns>
    public static Vector2 GetScreenSpacePosition(this RectTransform rectTransform, bool fromCorner = false)
    {
        Assert.IsTrue(rectTransform.rotation == Quaternion.identity, "RectTransform is rotated. This position only make sense for un-rotated tarnsforms.");

        if (fromCorner)
        {
            return new Vector2(rectTransform.position.x - (rectTransform.rect.width * rectTransform.pivot.x * rectTransform.lossyScale.x), rectTransform.position.y - rectTransform.rect.height * rectTransform.pivot.y * rectTransform.lossyScale.y);
        }
    ...
    public static Vector2 GetScreenSpaceDimensions(this RectTransform rectTransform)
    {
        Assert.IsTrue(rectTransform.rotation == Quaternion.identity, "RectTransform is rotated. These dimensions only make sense for un-rotated tarnsforms.");

        return new Vector2(rectTransform.rect.width * rectTransform.lossyScale.x, rectTransform.rect.height * rectTransform.lossyScale.y);
    }
    ...
    public static float GetScreenSpaceWidth(this RectTransform rectTransform)
    {
        Assert.IsTrue(rectTransform.rotation == Quaternion.identity, "RectTransform is rotated. These dimensions only make sense for un-rotated tarnsforms.");
    ...
    public static float GetScreenSpaceHeight(this RectTransform rectTransform)
    {
        Assert.IsTrue(rectTransform.rotation == Quaternion.identity, "RectTransform is rotated. These dimensions only make sense for un-rotated tarnsforms.");
    ...
    {
        Assert.IsTrue(rectTransform.rotation == Quaternion.identity, "RectTransform is rotated. This rect only make sense for un-rotated tarnsforms.");

        return new Rect(rectTransform.GetScreenSpacePosition(true), rectTransform.GetScreenSpaceDimensions());
    }
    ...
    public static bool ContainsScreenSpacePoint(this RectTransform rectTransform, Vector2 point)
    {
        Assert.IsTrue(rectTransform.rotation == Quaternion.identity, "RectTransform is rotated. This rect only make sense for un-rotated tarnsforms.");

        return new Rect(rectTransform.GetScreenSpacePosition(true), rectTransform.GetScreenSpaceDimensions()).Contains(point);
    }
    ...
    public static Vector2 GetNormalizedPositionOfScreenSpacePoint(this RectTransform rectTransform, Vector2 point)
    {
        Assert.IsTrue(rectTransform.rotation == Quaternion.identity, "RectTransform is rotated. This rect only make sense for un-rotated tarnsforms.");

        Rect rect = rectTransform.GetScreenSpaceRect();
