...
public class FSM<T> where T : struct, IConvertible, IComparable
{
    ...
    private Coroutine _startNextFrameRoutine;
    ...
    /// <summary>
    /// The previous state. Can be null if the state has never changed.
    /// </summary>
    ...
    /// </summary>
    public int NumRegisteredStates { get { return _states.Count; } }
    ...
    public void Start()
    {
        if (!_started)
        {
            ...
            if (_startNextFrameRoutine != null)
            {
                _component.StopCoroutine(_startNextFrameRoutine);
                _startNextFrameRoutine = null;
            }
        }
    }
    ...
    public FSM(MonoBehaviour component, bool autoRegister = true)
    {
        ...
        _startNextFrameRoutine = _component.StartCoroutine(StartNextFrame());
    }
    ...
    /// <param name="autoRegister">Whether or not to automatically register the state delegates using reflection</param>
    public FSM(MonoBehaviour component, T initialState, bool autoRegister = true)
    {
        DebugUtils.AssertIsEnumType<T>();
        _component = component;
